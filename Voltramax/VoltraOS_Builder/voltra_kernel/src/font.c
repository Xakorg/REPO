...
// Font array for printable ASCII characters (Space to ~)
const uint8_t font8x8_basic[95][8] = {
    ...
    {0x7F, 0x46, 0x16, 0x7E, 0x16, 0x46, 0x7F, 0x00}, // U+0045 (E)
    {0x7F, 0x46, 0x16, 0x7E, 0x16, 0x06, 0x0F, 0x00}, // U+0046 (F)
    ...
    {0x07, 0x18, 0x18, 0x70, 0x18, 0x18, 0x07, 0x00}, // U+007D (})
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}  // U+007E (~)
};

// Draw a single character at coordinates with scaling
void draw_char_scaled(char c, uint32_t x, uint32_t y, uint32_t fg_color, uint32_t bg_color, uint32_t scale) {
    ...
    for (uint32_t cy = 0; cy < 8; cy++) {
        for (uint32_t cx = 0; cx < 8; cx++) {
            // Read bits from right to left (LSB first) since font is stored that way!
            if (glyph[cy] & (1 << cx)) {
                draw_rect(x + (cx * scale), y + (cy * scale), scale, scale, fg_color);
            } else {
                if (bg_color != 0) { // If transparent bg is not desired, we could fill it.
...
// Draw an entire string of text with scaling
void draw_string_scaled(const char* str, uint32_t x, uint32_t y, uint32_t fg_color, uint32_t bg_color, uint32_t scale) {
    ...
    for (uint32_t i = 0; i < strlen(str); i++) {
        if (str[i] == '\n') {
            ...
            draw_char_scaled(str[i], x, y, fg_color, bg_color, scale);
            x += (8 * scale); 
        }
    }
}
