...
export default function ChatDashboardPage() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col bg-[#05030d] text-foreground animate-fade-in overflow-y-auto pb-20 md:pb-40">
        <section className="flex flex-col items-center justify-center text-center px-6 relative overflow-hidden pt-32 pb-40">
          ...
          >
            ...
            <div className="space-y-6">
              <h1 className="text-3xl sm:text-5xl md:text-7xl lg:text-[9rem] font-black tracking-tighter uppercase italic leading-[0.85] text-white drop-shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                Everything <br />
                <span className="text-emerald-500 flex items-center justify-center gap-4">Connects</span>
              </h1>
              <p className="text-base sm:text-xl md:text-4xl text-muted-foreground font-bold uppercase tracking-[0.2em] max-w-4xl mx-auto italic opacity-60">
                A hybrid social, gaming, and professional experience.
              </p>
            </div>
            <div className="pt-10">
              <Link href="/auth">
                <Button className="h-12 sm:h-16 md:h-24 px-6 sm:px-12 md:px-20 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[1.5rem] md:rounded-[2.5rem] font-black text-base sm:text-lg md:text-2xl uppercase italic shadow-[0_18px_60px_rgba(16,185,129,0.35)] hover:shadow-[0_30px_90px_rgba(16,185,129,0.5)] transition-all duration-300 active:scale-95 border-b-[8px] md:border-b-[12px] border-emerald-800 active:border-b-0 hover:-translate-y-1">
                  Join the Hub
                </Button>
              </Link>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 relative z-10">
          {FEATURE_CATEGORIES.map((cat, idx) => (
            <motion.div
              ...
              className="h-full"
            >
              <Card className="h-full glass-card p-6 sm:p-8 md:p-12 rounded-2xl sm:rounded-[3rem] md:rounded-[4rem] border-white/5 space-y-8 group hover:border-emerald-500/40 transition-all bg-black/40 shadow-2xl relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className={cn("w-16 h-16 sm:w-20 sm:h-20 rounded-xl sm:rounded-[2.5rem] bg-zinc-900 flex items-center justify-center border-4 border-white/10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-xl relative z-10", cat.color)}>
                  <cat.icon className="w-10 h-10" />
                </div>
                <div className="space-y-4 relative z-10">
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-black uppercase italic text-white tracking-tight">{cat.title}</h3>
                  <ul className="space-y-3">
                    {cat.features.slice(0, 5).map(f => (
                      <li key={f} className="flex items-center gap-3 text-[11px] sm:text-xs font-bold text-muted-foreground/80 italic">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]" /> {f}
                      </li>
                    ))}
                    <li className="text-[9px] sm:text-[10px] font-black uppercase text-primary tracking-widest pt-2">...and much more</li>
                  </ul>
                </div>
              </Card>
            </motion.div>
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center p-4 sm:p-8 bg-zinc-950 animate-fade-in relative">
      ...
      >
        <motion.div 
          ...
          className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl sm:rounded-[2.5rem] bg-black/40 border-4 border-white/10 flex items-center justify-center mx-auto shadow-[0_0_30px_rgba(255,255,255,0.08)] hover:shadow-[0_0_60px_rgba(16,185,129,0.25)] transition-shadow duration-500"
        >
          <MessageSquare className="w-12 h-12 text-white" style={{ stroke: "url(#mesh-gradient)", fill: "url(#mesh-gradient)", fillOpacity: 0.15 }} />
        </motion.div>
        <div className="space-y-3">
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-black uppercase italic tracking-tighter text-white">Welcome, {cleanDisplayName}!</h1>
