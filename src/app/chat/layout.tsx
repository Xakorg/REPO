    ...
    <div className="h-[calc(100vh-80px)] flex overflow-hidden bg-zinc-950 text-white relative">
      ...
      <aside className="hidden md:flex w-20 bg-[#05030d] border-r border-white/5 flex-col items-center py-6 gap-4 z-30 shrink-0">
        <ScrollArea className="w-full flex-1" style={{ height: "100%" }}>
          <div className="flex flex-col items-center gap-4 py-4 w-full">
            ...
            <div className="w-8 h-px bg-white/5 mx-auto shrink-0" />
            <button 
              onClick={() => setShowCreateServerModal(true)}
              className="w-12 h-12 rounded-full bg-white/5 text-emerald-500 flex items-center justify-center hover:bg-emerald-500 hover:text-black transition-all shrink-0"
            >
      ...
      <aside className="hidden md:flex w-64 bg-[#0a0a15] border-r border-white/5 flex-col z-20 shrink-0">
        {activeServer === 'home' ? (
          ...
          <>
            <header className="h-16 border-b border-white/5 px-6 flex items-center justify-between shadow-xl">
               <h2 className="text-sm font-black uppercase italic tracking-tighter text-white truncate">{serverHeaderTitle}</h2>
            </header>
            <ScrollArea className="flex-1">
                      ...
                      <div key={cat} className="space-y-1">
                        ...
                        {!isCollapsed && catChannels.map(ch => {
                          ...
                          const isActive = isVoice 
                            ? activeVoiceChannel === ch.name 
                            : pathname.includes(`/chat/s/${activeServer}`) && searchParams.get("c") === ch.name;
                              ...
                              <div key={ch.id} className="space-y-1">
                                <button 
                                  ...
                                  className={cn(
                                    ...
                                    isActive ? "bg-emerald-500/10 text-emerald-400 font-bold" : "text-white/40 hover:bg-white/5 hover:text-white"
                                  )}
                                >
                                    ...
                                    {voiceUsers[ch.name].map((vu: any) => {
                                      ...
                                      return (
                                        <div key={vu.uid} className="flex items-center gap-2 py-1">
                                          <Avatar className={cn("w-5 h-5 border transition-all", isSpeaking ? "border-emerald-500 ring-2 ring-emerald-500/40" : "border-white/5")}>
                                            <AvatarImage src={vu.photoURL} />
                                            <AvatarFallback className="text-[8px]">{vu.displayName?.[0]}</AvatarFallback>
                                          </Avatar>
        ...
        {voiceConnectedMsg && activeVoiceChannel && (
          <div className="p-3 bg-emerald-950/40 border-t border-b border-emerald-500/10 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span></span>
              <div>
                <p className="text-[9px] font-black text-emerald-400">RTC Connected</p>
                <p className="text-[8px] text-zinc-400 truncate max-w-[100px]">{activeVoiceChannel}</p>
              </div>
            </div>
            <button onClick={handleLeaveVoice} className="px-2 py-1 rounded bg-white/5 text-[8px] font-black uppercase hover:bg-red-500 hover:text-white transition-all">Leave</button>
          </div>
        )}
      ...
      <aside className="hidden xl:flex w-72 bg-[#0a0a15] border-l border-white/5 flex-col z-20 shrink-0">
        <Tabs value={rightPanel} onValueChange={(v: any) => setRightPanel(v)} className="h-full flex flex-col">
          ...
          <TabsContent value="members" className="flex-1 overflow-hidden m-0 flex flex-col">
             {!isDmRoute && (
                  ...
                  <div className="space-y-6 text-left">
                    {!isBuiltInServer ? (
                      <div className="space-y-2">
                        ...
                        {filteredCustomMembers.map(m => {
                          const name = m.displayName?.replace(/^@+/, "") || "Member";
                          const roleDetails = getMemberRoleDetails(m.id);
                          const isSpeaking = speakingUsers.includes(m.id);
                          return (
                            <div 
                              key={m.id} 
                              onClick={() => setSelectedProfileUser(m)}
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                           <p className="px-3 text-[9px] font-black uppercase text-muted-foreground tracking-widest mb-3">Online — {filteredHubMembers.length}</p>
