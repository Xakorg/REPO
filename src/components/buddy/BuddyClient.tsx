...
export default function BuddyClient() {
  ...
  const { data: allBuddies } = useCollection(buddiesQuery);

  const myPets = allBuddies?.filter((b: any) => b.ownerId === user?.uid) || [];
  const onlinePets = allBuddies?.filter((b: any) => b.ownerId !== user?.uid) || [];

  const activePet = myPets.find((p: any) => p.id === selectedPetId) || myPets[0];
              ...
              <Card className="glass-card rounded-[3rem] p-8 border-4 border-white/10 h-full flex flex-col shadow-xl">
                ...
                <ScrollArea className="flex-1">
                  <div className="space-y-6 pr-4">
                    {allBuddies?.slice(0, 15).map((pet: any) => (
