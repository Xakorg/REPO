...
export default function ReactionTime() {
  ...
  const msg =
    ...
    `${times[times.length - 1]}ms — Click again`;
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-zinc-950 gap-8">
      <div className="text-center">
        ...
        {times.length > 0 && (
          <p className="text-zinc-400">Avg: <span className="text-cyan-400 font-bold">{avg}ms</span></p>
        )}
      </div>
      ...
      {times.length > 0 && (
        <div className="flex gap-3">
          {times.map((t, i) => (
            <div
              key={i}
              className={`px-4 py-2 rounded-xl text-white font-black text-sm border-2 ${
                t < 200 ? "border-emerald-500 bg-emerald-500/10" :
                t < 350 ? "border-cyan-500 bg-cyan-500/10" :
      ...
      <div className="text-zinc-600 text-xs text-center">
        <p>Green = ready, click as fast as possible!</p>
        <p>If you click during red = too early!</p>
      </div>
    </div>
  );
}
