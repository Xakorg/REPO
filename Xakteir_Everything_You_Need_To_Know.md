...
## AI Chat
- **Xakteir Labs (`labs.xakteir.com`):** Experimental R&D sandbox containing Knowledge Engine 2.0, AI Voice Synthesizer, 3D Quantum Matrix Simulator, Holographic Spatial Web, and Xak Code AI Sandbox.
- **Knowledge Engine 2.0:** AI study & research hub with article synthesis, interactive flashcards, topic mind-maps, and AI quiz engine.
- **2FA Auth & Discord-Style Profile/Settings:** Overhauled Auth with 2FA verification step. Clean separation between `/profile` (identity, cosmetics, bio, badges) and `/settings` (2FA choices, security, linked accounts, notifications, themes).
